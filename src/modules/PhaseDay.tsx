...
// Split a single "big day" into multiple days when items contain lines like "Dag 2 – ..."
  ...
  const dayIndex = Math.max(
    ...
    days.findIndex(
      (d: any) =>
        ...
        String(d.title).toLowerCase().includes(String(dayId).toLowerCase())
    )
  );
  const day = days[dayIndex] || days[0];

  const [checks, setChecks] = useLocalState<Record<string, boolean>>(
    `checks:fase${num}:day:${day.id}`,
    {}
  );
  const [note, setNote] = useLocalState<string>(
    `notes:fase${num}:day:${day.id}`,
    ""
  );
